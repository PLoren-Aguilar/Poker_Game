import time
from include.holdem_probs import holdem_functions
from include.holdem_probs import holdem_argparser
...
def run_simulation(hole_cards, num, exact, given_board, deck, verbose):
    ...
    for player_index, histogram in enumerate(result_histograms):
        ...
        for index, elem in enumerate(histogram):
            freqs.append(float(elem) / float_iterations)
        print("")
        results.append([player_index,freqs])                        
    percentages = holdem_functions.find_winning_percentage(winner_list)
